...
private var heart : HeartBeat;		//reference to script/actions of heartbeat
...
function Start(){
	...
	for (var child : Transform in transform) {
		...
		if (child.name == "graphPanel"){
			...
			child.renderer.material.color.a = gameObject.renderer.material.color.a = 0.25;
		...
		if (child.name == "graphText"){
			...
			for (var tex : TextMesh in texts) {
				if(tex.name == "text_patientNumber"){
					...
					var L = Letters[R];
					var P:int = X%10 +1;
					tex.text = "POD1 | "+P+L;//((number<10)?"0":"")+number;
				}//Set Patient location, ex: 1P1R1F
			}//Ends loop
		}//Ends check
		if (child.name == "heartRate"){
			heartRate = child.gameObject;
			heart = heartRate.GetComponent(HeartBeat);
			heart.num = number;
			heart.mon = mon;
			heart.colV = colV;
			heart.graphType = grapher.graphType;
		}//Ends check
...
function setHighlight(value : boolean){
	...
	if(value){
		...
		for (var child in graphPanel.GetComponentsInChildren(Renderer)) {
			child.renderer.material.color.a = gameObject.renderer.material.color.a = 1;
...
function resizeAction(value : boolean){
	if(value){
	//	graphPanel.transform.localScale *= 1.1f;
		//graphPanel.transform.localPosition.x += scale/2.0f;
		graphPanel.transform.localScale += Vector3(0.01,0.01,0);
