...
public class MouseCameraControlP6LargeScreen : MonoBehaviour {
	...
	void LateUpdate ()
	{
		var SmartScreen = GameObject.Find("SmartScreen");
	...
	//	print(Monitor1);
       ...
       // if (verticalTranslation.isActivated())
		...
		transform.position = new Vector3(Mathf.Clamp(transform.position.x, (SmartScreen.transform.renderer.bounds.min.x + 0.05f), (SmartScreen.transform.renderer.bounds.max.x - 0.05f)), Mathf.Clamp(transform.position.y, SmartScreen.transform.renderer.bounds.min.y, SmartScreen.transform.renderer.bounds.max.y - .09f), SmartScreen.transform.renderer.bounds.max.z);
